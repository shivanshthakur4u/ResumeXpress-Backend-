...
router.post("/generate/:kind", aiLimiter, validate(z.object({ params: z.object({ kind: z.enum(["match", "optimizer", "bullets", "summary", "skills", "linkedin", "gap", "coach"]) }), body: schemas.contextInput })), respond(async req => ({ analysis: await service.generate(req.user.email, req.params.kind, req.body) })));
